...
8 references across different domains. format: "Domain — Name" (e.g., "Music — Grouper"). no explanations. no parentheticals. at least 6 different domains. no more than 1 from the same domain group. domain groups: cinema (Film), music (Music, Album, Artist), literature (Book), architecture (Architect, Building), fashion (Brand, Designer). all other domains are their own group. every recommendation should be something the user has likely NOT encountered but will immediately recognize as theirs when they look it up. go deep, not broad. prefer the specific over the canonical.
